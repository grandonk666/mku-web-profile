...
<form method="POST" action="{{ route("admin.post.update", $post) }}" enctype="multipart/form-data" class="p-10 bg-white rounded shadow-md w-full mb-4">
  ...
  <div class="mb-4">
      ...
      <input class="w-full px-5 py-1 text-gray-800 bg-gray-200 rounded outline-none border-2 border-gray-200 focus:border-gray-800 @error("judul") border-red-500 @enderror" id="judul" name="judul" type="text" placeholder="Judul" maxlength="50" aria-label="judul" value="{{ old("judul", $post->judul), }}">
      @error("judul")
