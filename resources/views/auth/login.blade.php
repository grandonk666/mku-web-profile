...
<section class="min-h-screen flex items-stretch text-black">
  <div class="lg:flex w-3/5 hidden bg-no-repeat bg-cover relative items-center"
    style="background-image: url({{ asset("home-hero.jpg") }});">
    <div class="absolute bg-black opacity-60 inset-0 z-0"></div>
    <div class="w-full px-24 z-10 text-white">
      <h1 class="text-5xl font-bold text-left tracking-wide">Matakuliah Umum
      </h1>
      <p class="text-3xl my-4">UPN "Veteran" Jawa Timur</p>
    </div>
  </div>
  <div
    class="lg:w-2/5 w-full flex items-center justify-center text-center md:px-16 px-0 z-0 bg-white">
    <div
    ...
    <div class="w-full py-6 z-20">
      <h1
        class="text-5xl tracking-wider font-bold mb-8 text-white lg:text-gray-700 uppercase">
        Login</h1>
      ...
      <form action="{{ route("authenticate") }}" method="POST"
        ...
        <div class="w-full px-4 py-8 bg-white/80 rounded-md">
          <div class="relative mb-8">
            <input id="email" type="email" name="email"
              class="peer h-10 w-full border-b-2 text-gray-900 placeholder-transparent focus:outline-none focus:border-indigo-500 bg-transparent @error("
