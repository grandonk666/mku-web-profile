...
<div class="relative w-full min-h-[70vh] md:min-h-screen flex justify-center items-center bg-cover bg-bottom" style="background-image: url({{ asset("home-hero.jpg") }})">
  <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent bg-black/50 flex justify-center items-center flex-col">
    ...
    <a href="{{ route("matakuliah") }}" class="px-5 py-3 bg-white text-lg font-bold rounded border-2 border-white hover:bg-transparent hover:text-white transition-all">Daftar Matakuliah</a>
  </div>
</div>
<main class="container mx-auto min-h-screen py-12 px-4">
  <div class="w-full flex flex-col md:flex-row justify-between md:items-center mb-8">
    ...
    <a href="{{ route("posts.index") }}" class="flex items-center text-gray-800 hover:text-gray-500">
        ...
        <div class="bg-white h-full p-6 -mt-24 md:mt-0 relative md:mb-0 flex flex-col items-center justify-between">
          <div class="w-full text-center md:text-left">
            ...
            <p class="mb-0 mt-1 text-blue-600 text-sm uppercase">
              <a href="#" class="hover:underline">{{ $post->kategori->nama }}</a> - <span class="text-gray-500 italic capitalize lining-nums">{{ $post->created_at->format('d M Y') }}</span>
            </p>
          ...
          <div class="w-full mt-4 text-center md:text-left">
            <a class="text-blue-400 hover:text-blue-900 text-sm transition duration-150 ease-in-out group" href="{{ route("posts.show", $post) }}">Read more
